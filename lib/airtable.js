...
/**
 * Escape a value for use in Airtable filterByFormula strings.
 ...
 * @returns {Object|null} Member record or null if not found / on error.
 * @throws Never — errors are caught and logged; returns null.
 */
 ...
 * @throws {Error} On Airtable API failure.
 */
...
export async function getActiveDropsByMember(memberId) {
  try {
    const data = await airtableFetch(TABLES.DROPS, {
      params: {
        filterByFormula: `AND(
          FIND('${escapeFormulaValue(memberId)}', ARRAYJOIN({Member})),
          NOT(OR(
            {Status} = '${escapeFormulaValue(DROP_STATUSES.COLLECTED)}'
          ))
        )`,
...
export async function getAllDropsByMember(memberId, limit = 20) {
  try {
    const data = await airtableFetch(TABLES.DROPS, {
      params: {
        filterByFormula: `FIND('${escapeFormulaValue(memberId)}', ARRAYJOIN({Member}))`,
        sort: [{ field: 'Drop Date', direction: 'desc' }],
        maxRecords: limit,
      },
    });
...
export async function getGymByCode(gymCode) {
  ...
  try {
    const data = await airtableFetch(TABLES.GYMS, {
      params: {
        filterByFormula: `LOWER({Code}) = '${escapeFormulaValue(gymCode.toLowerCase())}'`,
        maxRecords: 1,
      },
    });
