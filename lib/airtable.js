...
export async function airtableFetch(endpoint, options = {}) {
  let paramStr = '';
  if (options.params) {
...
export async function createIssue({ memberId, type, description, photoUrls = [], dropId = null, source = 'WhatsApp' }) {
  try {
...
export async function getIssueById(issueId) {
  try {
    ...
    return { id: data.id, fields: data.fields };
  } catch (e) { console.error('getIssueById:', e); return null; }
}
...
export async function checkVerificationCode(memberId, code) {
  ...
  await updateMember(memberId, { 'Verification Code': '', 'Verification Expires': null, 'Verification Attempts': 0 });
  return { success: true, member };
}

export async function clearVerificationCode(memberId) {
  return updateMember(memberId, { 'Verification Code': '', 'Verification Expires': null, 'Verification Attempts': 0 });
}
