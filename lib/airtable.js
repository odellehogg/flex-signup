...
async function airtableFetch(endpoint, options = {}) {
  ...
  const url = `${BASE_URL}/${encodeURIComponent(endpoint)}${paramStr}`;
      ...
      body: {
        records: [{
          fields: {
            ...
            'Drops Allowed': memberData.dropsRemaining || 0,
            'Drops Used': 0,
          },
        }],
      },
...
// Helper: read drops remaining from actual field 'Drops Allowed'
...
export async function getActiveDropsByMember(memberId) {
  try {
    const data = await airtableFetch(TABLES.DROPS, {
      params: {
        filterByFormula: `AND(FIND('${escapeFormulaValue(memberId)}',ARRAYJOIN({Member})),NOT({Status}='${DROP_STATUSES.COLLECTED}'))`,
