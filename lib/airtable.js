...
// Using direct fetch API to avoid Airtable SDK AbortSignal issues
...
export async function createMember(data) {
  try {
    return await createRecord('Members', {
...
export async function createDrop(memberId, bagNumber, gymId) {
  try {
    ...
    const record = await createRecord('Drops', dropData)
    ...
    if (error.message.includes('Unknown field') || error.message.includes('Invalid')) {
      ...
      return await createRecord('Drops', retryData)
    }
...
export async function getAllDrops(filters = {}) {
  try {
    ...
    return await getRecords('Drops', options)
  } catch (error) {
...
export async function getDropCountsByStatus() {
  try {
    const records = await getRecords('Drops', { maxRecords: 500 })
...
export async function getGymById(gymId) {
  try {
    const record = await getRecord('Gyms', gymId)
    return {
      id: record.id,
...
export async function createIssue(memberId, type, description, photoUrl = null) {
  try {
    ...
    const record = await createRecord('Issues', issueData)
...
export async function getPlans() {
  try {
    const records = await getRecords('Plans', {
      filterByFormula: `{Is Active} = TRUE()`,
...
export async function createAuditLog(action, actor, entityType, entityId, details = '', ipAddress = '') {
  try {
    return await createRecord('Audit Logs', {
...
export async function getOpsDashboardData() {
  try {
    const [members, drops, issues] = await Promise.all([
      getRecords('Members', { maxRecords: 500 }),
      getRecords('Drops', { maxRecords: 500 }),
      getRecords('Issues', { maxRecords: 100 }),
    ])
...
export async function getLaundryPartners() {
  try {
    return await getRecords('Laundry Partners')
  } catch (error) {
...
export async function getGymLaundryMapping() {
  try {
    const records = await getRecords('Gyms Laundry Mapping')
    const mapping = {}
...
export async function getRevenueByPlan() {
  try {
    const [members, plans] = await Promise.all([
      getRecords('Members', { filterByFormula: `{Subscription Status} = 'Active'` }),
      getRecords('Plans'),
    ])
...
export async function getChurnStats() {
  try {
    const members = await getRecords('Members', {
...
export async function getSLAStats() {
  try {
    const drops = await getRecords('Drops', {
...
export async function getBagsBreachingSLA() {
  try {
    const drops = await getRecords('Drops', { maxRecords: 500 })
    const breaches = []
