...
export async function getMemberByStripeId(stripeCustomerId) {
  try {
    ...
    console.error('getMemberByStripeId error:', error)
    return null
  }
}
...
export async function getInactiveMembers(days = 14) {
  ...
  try {
    ...
    console.error('getInactiveMembers error:', error)
    return []
  }
}
...
// Get active drop for a member - fetch all active drops and filter in code
export async function getActiveDropByMember(memberId) {
  try {
    ...
    // Filter by member ID in code since linked record filtering is tricky
    const memberDrops = result.records?.filter(drop => {
      const memberField = drop.fields['Member']
      return Array.isArray(memberField) && memberField.includes(memberId)
    }) || []
    
    // Sort by drop date descending and return first
    memberDrops.sort((a, b) => new Date(b.fields['Drop Date'] || 0) - new Date(a.fields['Drop Date'] || 0))
    
    return memberDrops[0] || null
  } catch (error) {
    console.error('getActiveDropByMember error:', error)
    return null
  }
}

// Get drop that's awaiting pickup confirmation (status = Ready)
export async function getDropAwaitingPickupConfirm(memberId) {
  try {
    ...
    // Filter by member ID in code
    const memberDrops = result.records?.filter(drop => {
      const memberField = drop.fields['Member']
      return Array.isArray(memberField) && memberField.includes(memberId)
    }) || []
    
    // Sort by ready date descending and return first
    memberDrops.sort((a, b) => new Date(b.fields['Ready Date'] || 0) - new Date(a.fields['Ready Date'] || 0))
    
    return memberDrops[0] || null
  } catch (error) {
    console.error('getDropAwaitingPickupConfirm error:', error)
    return null
  }
}
...
export async function getDropsNeedingPickupConfirmation() {
  ...
  try {
    ...
    console.error('getDropsNeedingPickupConfirmation error:', error)
    return []
  }
}
...
export async function getDropsNeedingPickupReminder() {
  ...
  try {
    ...
    console.error('getDropsNeedingPickupReminder error:', error)
    return []
  }
}
...
export async function getMemberDropsThisMonth(memberId) {
  ...
  try {
    const result = await airtableRequest('Drops', {
      params: {
        filterByFormula: `{Drop Date} >= '${startOfMonth}'`,
      },
    })
    ...
    return memberDrops.length
  } catch (error) {
    console.error('getMemberDropsThisMonth error:', error)
    return 0
  }
}
...
export async function getStuckDrops() {
  ...
  try {
    ...
    console.error('getStuckDrops error:', error)
    return []
  }
}
...
export async function getOverduePickups() {
  ...
  try {
    ...
    console.error('getOverduePickups error:', error)
    return []
  }
}
...
export async function getMembersWithPaymentIssues() {
  try {
    ...
    console.error('getMembersWithPaymentIssues error:', error)
    return []
  }
}
...
export async function updateDropScanLog(dropId, scanEntry) {
  try {
    ...
    console.error('updateDropScanLog error:', error)
    return null
  }
}
...
export async function createIssue(memberId, type, description, photoUrl = null, priority = 'Medium', dropId = null) {
  ...
  try {
    ...
    return { fields: { 'Ticket ID': ticketId } }
  }
}
...
export async function getOpsDashboardData() {
  try {
    ...
    try {
      ...
      dropsThisMonth = drops.records?.length || 0
    } catch {
      dropsThisMonth = 0
    }

    let allDrops = { records: [] }
    try {
      allDrops = await airtableRequest('Drops')
    } catch {
      allDrops = { records: [] }
    }
...
export async function getDropsAwaitingPickup() {
  try {
    ...
    console.error('getDropsAwaitingPickup error:', error)
    return []
  }
}
...
export async function getDropsInTransit() {
  try {
    ...
    console.error('getDropsInTransit error:', error)
    return []
  }
}
...
export async function getDropsAtLaundry() {
  try {
    ...
    console.error('getDropsAtLaundry error:', error)
    return []
  }
}
...
export async function getDropsReadyForDelivery() {
  try {
    ...
    console.error('getDropsReadyForDelivery error:', error)
    return []
  }
}
...
export async function getMemberBag(memberId) {
  try {
    const result = await airtableRequest('Bags', {
      params: {
        filterByFormula: `{Status} = 'Issued'`,
      }
    })
    
    // Filter by member ID in code
    const memberBags = result.records?.filter(bag => {
      const memberField = bag.fields['Member']
      return Array.isArray(memberField) && memberField.includes(memberId)
    }) || []
    
    return memberBags[0] || null
  } catch (error) {
...
export async function getSLAStats() {
  try {
    ...
    completedDrops.records?.forEach(drop => {
      ...
      if (!collectedDate || collectedDate < thirtyDaysAgo) return
