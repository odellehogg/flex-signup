...
export async function getMemberByPhone(phone) {
  ...
  let normalizedPhone = phone.replace('whatsapp:', '')
...
// NEW: Get drop that's awaiting pickup confirmation (status = Ready, not yet confirmed)
...
// NEW: Get drops ready for pickup confirmation request (24 hours since ready)
...
// NEW: Get drops needing pickup reminder (48 hours since ready, already sent confirm request)
...
export async function getStuckDrops() {
  ...
  const result = await airtableRequest('Drops', {
    params: {
      filterByFormula: `OR(
        AND({Status} = 'Dropped', {Drop Date} < '${sixHoursAgoStr}'),
        AND({Status} = 'At Laundry', {Drop Date} < '${thirtyHoursAgoStr}')
      )`,
    },
  })
...
// Each gym can be assigned to a specific laundry partner
export async function getGymLaundryMapping() {
  const gyms = await getGyms()
  const mapping = {}
  
  gyms.forEach(gym => {
    const gymName = gym.fields['Name']
...
// ============================================================================
// BAG MANAGEMENT
// Bags are issued when customer wants to drop, returned when inactive/pausing
// ============================================================================
...
export async function getAvailableBags() {
  const result = await airtableRequest('Bags', {
    params: {
      ...
      sort: [{ field: 'Bag Number', direction: 'asc' }],
    }
  })
  return result.records || []
}
...
export async function issueBagToMember(bagId, memberId) {
  const result = await airtableRequest(`Bags/${bagId}`, {
    method: 'PATCH',
    body: {
      fields: {
        'Status': 'Issued',
...
export async function getBagsWithIssues() {
  const result = await airtableRequest('Bags', {
    params: {
      ...
      sort: [{ field: 'Unreturned Date', direction: 'desc' }],
    }
  })
  return result.records || []
}
...
export async function getBagsBreachingSLA() {
  ...
  allDrops.records?.forEach(drop => {
    ...
    // SLA thresholds
    const thresholds = {
      'Dropped': 6,        // Should be picked up within 6 hours
      'At Laundry': 30,    // Should be cleaned within 30 hours
      'Ready': 120,        // Should be collected within 5 days (120 hours)
    }
...
export async function getSLAStats() {
  ...
  const completedDrops = await airtableRequest('Drops', {
    params: {
      filterByFormula: `AND({Status} = 'Collected', IS_AFTER({Collected Date}, '${thirtyDaysAgo.toISOString()}'))`,
    }
  })
  ...
  completedDrops.records?.forEach(drop => {
    ...
    const collectedDate = new Date(drop.fields['Collected Date'])
...
export async function getChurnStats() {
  ...
  // Get cancelled members in last 30 days
  const cancelled = await airtableRequest('Members', {
    params: {
      filterByFormula: `AND({Status} = 'Cancelled', IS_AFTER({Cancelled Date}, '${thirtyDaysAgo.toISOString()}'))`,
    }
  })
  ...
  cancelled.records?.forEach(member => {
    const reason = member.fields['Cancel Reason'] || 'Unknown'
...
export async function getUtilisationStats() {
  ...
  for (const member of members.records || []) {
    const planName = member.fields['Subscription Tier'] || 'Essential'
    const allowed = planName === 'Unlimited' ? 16 : 10 // TODO: fetch from plan
    totalAllowed += allowed

    // Count drops this month
    const drops = await airtableRequest('Drops', {
      params: {
        filterByFormula: `AND({Member} = '${member.id}', IS_AFTER({Drop Date}, '${startOfMonth.toISOString()}'))`,
      }
    })
    totalUsed += drops.records?.length || 0
  }
