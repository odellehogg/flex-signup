 ...
 */
export async function verifyLoginCode(memberId, code) {
  const result = await verifyAirtableCode(memberId, code);

  if (!result.success) {
    return result;
  }

  // Get member details for token
...
export function createAuthCookie(token) {
  ...
  return `flex_token=${token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${7 * 24 * 60 * 60}${isProduction ? '; Secure' : ''}`;
}
...
export function createLogoutCookie() {
  return `flex_token=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0`;
}
