...
import { getMemberByPhone, getMemberById, setVerificationCode, verifyCode as verifyAirtableCode, normalizePhone } from './airtable.js';
import { sendVerificationCodeMessage } from './whatsapp.js';
...
export async function requestLoginCode(phoneOrEmail) {
  ...
  if (sendVia === 'whatsapp') {
    const memberPhone = member.fields?.['Phone'] || member['Phone'];
    try {
      await sendVerificationCodeMessage(memberPhone, code);
    } catch (err) {
      console.error('[Auth] WhatsApp verification failed:', err.message);
      // Don't throw — email backup will still be sent below
    }
  }

  // Also send code via email as backup
...
export async function verifyLoginCode(phoneOrMemberId, code) {
  ...
  const token = generateToken({
    memberId: member.id,
    phone: member.fields?.['Phone'] || member['Phone'],
  ...
  return {
    ...
    member: {
      ...
      status: member.fields?.['Subscription Status'] || member['Subscription Status'],
    },
  };
}
