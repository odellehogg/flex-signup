...
// TEMPLATE_VERIFY_CODE_SID     = (create flex_verify_code)        — vars: {{1}}=code
// TEMPLATE_LOW_DROPS_SID       = (create flex_low_drops)          — vars: {{1}}=firstName, {{2}}=dropsRemaining, {{3}}=daysLeft
// TEMPLATE_PAYMENT_FAILED_SID  = (create flex_payment_failed)     — vars: {{1}}=firstName
// TEMPLATE_DROPS_RESET_SID     = (create flex_drops_reset)        — vars: {{1}}=firstName, {{2}}=dropsAllowed
// TEMPLATE_ADDON_CONFIRMED_SID = (create flex_addon_confirmed)    — vars: {{1}}=firstName, {{2}}=dropsRemaining
// TEMPLATE_SUB_CANCELLED_SID   = (create flex_sub_cancelled)      — vars: {{1}}=firstName
