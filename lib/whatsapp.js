...
// PATCHED: Added plain text fallbacks when templates fail or aren't configured
...
async function sendTemplate(to, contentSid, variables = {}, templateKey = null) {
  ...
  // If no template SID configured, use plain text fallback
  if (!contentSid) {
    console.warn(`⚠️ Template SID not configured${templateKey ? ` for ${templateKey}` : ''}, using plain text`)
    ...
    // Generic fallback if no specific template
  ...
  try {
    ...
    console.log(`✅ Template sent: ${message.sid}`)
    return message
  } catch (error) {
    console.error(`❌ Template failed (${templateKey || 'unknown'}):`, error.message)
