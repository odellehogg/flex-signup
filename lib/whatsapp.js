...
export async function sendVerificationCodeMessage(to, code) {
  return sendWithFallback(
    to,
    process.env.TEMPLATE_VERIFY_CODE_SID,
    { '1': code },
...
export async function sendInvalidBagNumber(to, input, reason) {
  // flex_invalid_bag template is static (no vars) — shows generic bag number format message
  let message = `"${input}" `;
  switch (reason) {
    case 'INVALID_FORMAT':
  ...
  return sendWithFallback(
    to,
    process.env.TEMPLATE_INVALID_BAG_SID,
    {},
...
export async function sendReadyPickup(to, { bagNumber, gymName, firstName, pickupDeadline }) {
  // flex_ready_pickup vars: {{1}}=bagNumber, {{2}}=gymName, {{3}}=availableUntil
  const deadline = pickupDeadline || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
    .toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'short', timeZone: 'Europe/London' });
  return sendWithFallback(
    to,
    process.env.TEMPLATE_READY_PICKUP_SID,
    { '1': bagNumber, '2': gymName, '3': deadline },
    `Your clothes are ready${firstName ? `, ${firstName}` : ''}! 👕✨\n\n` +
    `📦 Bag ${bagNumber} is waiting at ${gymName} reception.\n\n` +
    `Available until ${deadline} — just ask staff for your FLEX bag.\n\n` +
...
export async function sendPauseConfirmed(to, { resumeDate }) {
  // flex_pause_confirmed: "Your subscription is now paused. It will automatically resume on {{1}}."
  return sendWithFallback(
    to,
    process.env.TEMPLATE_PAUSE_CONFIRMED_SID,
    { '1': resumeDate },
...
// flex_manage_sub: "Plan: {{1}} (£{{2}}/mo)\nNext billing: {{3}}"
export async function sendManageMenu(to, planName, status, { price, billingDate } = {}) {
  if (price && billingDate && process.env.TEMPLATE_MANAGE_SUB_SID) {
    return sendWithFallback(
      to,
      process.env.TEMPLATE_MANAGE_SUB_SID,
      { '1': planName, '2': String(price), '3': billingDate },
      `📋 Your subscription:\n\nPlan: ${planName}\nStatus: ${status}\n\nTo manage your subscription, visit:\n${COMPANY.website}/portal`
    );
  }
  return sendMessage(to,
...
// flex_pickup_confirmed_thanks: "Thanks for confirming! How was your FLEX experience?"
// Buttons: "Great" / "OK" / "Not Good"
export async function sendPickupConfirmedThanks(to) {
  return sendWithFallback(
    to,
    process.env.TEMPLATE_PICKUP_THANKS_SID,
    {},
    `Thanks for confirming! ✅\n\nHow was your FLEX experience this time?\n\n` +
    `Reply GREAT, OK, or NOT GOOD.`
  );
}

// flex_pickup_confirm_request: "Have you collected your bag? Bag {{1}} has been ready at {{2}} for 24 hours."
export async function sendPickupConfirmRequest(to, { bagNumber, gymName }) {
  return sendWithFallback(
    to,
    process.env.TEMPLATE_PICKUP_CONFIRM_SID,
    { '1': bagNumber, '2': gymName },
    `Have you collected your bag? 👋\n\n` +
    `Bag ${bagNumber} has been ready at ${gymName} for 24 hours.\n\n` +
    `Please confirm so you can make your next drop!\n\n` +
    `Reply YES if collected, or HELP if you need assistance.`
  );
}

// flex_pickup_reminder: "Reminder: Your bag is still waiting! Bag {{1}} is at {{2}} reception."
export async function sendPickupReminder(to, { bagNumber, gymName }) {
  return sendWithFallback(
    to,
    process.env.TEMPLATE_PICKUP_REMINDER_SID,
    { '1': bagNumber, '2': gymName },
    `Reminder: Your bag is still waiting! 📦\n\n` +
    `Bag ${bagNumber} is at ${gymName} reception.\n\n` +
    `Please collect it soon. Reply HELP if you need assistance.`
  );
}
...
// ============================================================================
// LOW DROPS / PAYMENT / ADDON / CANCELLATION TEMPLATES
// ============================================================================

export async function sendLowDrops(to, { firstName, dropsRemaining, daysLeft }) {
  return sendWithFallback(
    to,
    process.env.TEMPLATE_LOW_DROPS_SID,
    { '1': firstName, '2': String(dropsRemaining), '3': String(daysLeft) },
    `Heads up ${firstName} — you have *${dropsRemaining} drop${dropsRemaining !== 1 ? 's' : ''}* left with *${daysLeft} days* still to go this month.\n\n` +
    `Need more? Add an extra drop for £4:\n${COMPANY.website}/portal\n\n` +
    `Or reply EXTRA DROP for a payment link.`
  );
}

export async function sendPaymentFailed(to, { firstName }) {
  return sendWithFallback(
    to,
    process.env.TEMPLATE_PAYMENT_FAILED_SID,
    { '1': firstName },
    `Hi ${firstName}, your FLEX payment failed. 😕\n\n` +
    `Update your payment method to keep your subscription:\n${COMPANY.website}/portal\n\n` +
    `Need help? Reply HELP.`
  );
}

export async function sendDropsReset(to, { firstName, dropsAllowed }) {
  return sendWithFallback(
    to,
    process.env.TEMPLATE_DROPS_RESET_SID,
    { '1': firstName, '2': String(dropsAllowed) },
    `Hey ${firstName}! Your ${dropsAllowed} drops have been refreshed for this month. 🎉\n\n` +
    `Ready to make a drop? Reply 1 or text DROP!`
  );
}

export async function sendAddonConfirmed(to, { firstName, dropsRemaining }) {
  return sendWithFallback(
    to,
    process.env.TEMPLATE_ADDON_CONFIRMED_SID,
    { '1': firstName, '2': String(dropsRemaining) },
    `Your extra drop has been added, ${firstName}! 🎉\n\n` +
    `You now have ${dropsRemaining} drop(s) remaining this month.\n\n` +
    `Ready to use it? Reply 1 or type DROP.`
  );
}

export async function sendSubCancelled(to, { firstName }) {
  return sendWithFallback(
    to,
    process.env.TEMPLATE_SUB_CANCELLED_SID,
    { '1': firstName },
    `Hi ${firstName}, your FLEX subscription has ended. 😢\n\n` +
    `Hope to see you again! Rejoin anytime at:\n${COMPANY.website}`
  );
}
